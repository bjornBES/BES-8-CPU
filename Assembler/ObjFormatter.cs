...
namespace assembler
{
    public class ObjFormatter
    {
        ...
        {
            if (Data.StartsWith(Generation.LabelIdent))
            {
        ...
        public static void AddBytes(string[] ByteStr)
        {
            List<byte> bytes = new List<byte>();
            for (int i = 0; i < ByteStr.Length; i++)
            {
                bytes.Add(Encoding.ASCII.GetBytes(ByteStr[i])[0]);
            }
            InsertBytes(bytes.ToArray());
        }
        ...
        public static void AddInstr(string instr, params string[] args)
        {
            FormatAndPlace(instr);
            for (int i = 1; i < args.Length; i++)
            {
                if (!args[i].StartsWith(Generation.LabelIdent))
                {
                    FormatAndPlace(args[i]);
                }
                else if (args[i].StartsWith(Generation.LabelIdent))
                {
